...
namespace Fusee.Tutorial.Core
{
    ...
    public class Pick : RenderCanvas
    {
        ...
        SceneNodeContainer PickAtPosition(IEnumerable<SceneNodeContainer> listToSearchIn, float2 pickPosition, float4x4 mvp)
        {
            ...
            foreach (var child in listToSearchIn)
            {
                ...
                foreach (IEnumerable<TransformComponent> components in a)
                {
                    mvp = RC.ModelView * a.Matrix();
                }

                var tmesh = new Mesh();
                //float2[] transtriangle = new float2[tmesh.Triangles.Length];
                List<float2> transtriangle = new List<float2>();

                foreach (var triangle in tmesh)
                {
                    transtriangle.Add(triangle*mvp);//= triangle * mvp;
                }

                var _pickpos = Mouse.Position;

                for (int i = 0; i < transtriangle.Count; i++)
                {
                    //flächeninhalt Dreieck  0,5 * | (a-b)x(a-c) |
                    //Check if pickpos is part of flächeninhalt
                    //gausß aufstellen
